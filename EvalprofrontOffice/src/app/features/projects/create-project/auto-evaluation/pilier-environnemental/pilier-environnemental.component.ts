...
@Component({
  selector: 'app-pilier-environnemental',
  templateUrl: './pilier-environnemental.component.html',
  styleUrls: ['./pilier-environnemental.component.scss']
})
export class PilierEnvironnementalComponent {
  formGroup!: FormGroup;
  formMetadata: any;
  ...
  private setupRouteListener(): void {
    this.router.events.pipe(
      ...
      if (event.urlAfterRedirects.includes('/step3/environnemental')) {
        this.initForm();
        this.loadForm();
      }
    });
  ...
  private loadForm(): void {
    ...
    const handleForm = (form: any) => {
      ...
      form.questions.forEach((q: any) => {
        if ([39, 40, 41].includes(q.id)) {
          q.section = 'Env 1';
          q.pillar = 'ENVIRONNEMENTAL';
        } else if ([42, 43].includes(q.id)) {
          q.section = 'Env 2';
          q.pillar = 'ENVIRONNEMENTAL';
        } else if ([44, 45].includes(q.id)) {
          q.section = 'Env 3';
          q.pillar = 'ENVIRONNEMENTAL';
        } else if ([46, 47, 48].includes(q.id)) {
          q.section = 'Env Compléments';
          q.pillar = 'ENVIRONNEMENTAL';
        }
      });
  ...
  goToSection(section: number): void {
    const sectionLabels = ['Env 1', 'Env 2', 'Env 3', 'Env Compléments'];
  ...
  currentSectionLabel(): string {
    const labels = ['Env 1', 'Env 2', 'Env 3', 'Env Compléments'];
